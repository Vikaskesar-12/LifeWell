      ...
      <div style="display: flex; justify-content: flex-end; margin-bottom: 10px;">
            <form id="searchingForm" method="GET" style="width: 300px;">
                <div class="input-group">
                    <input type="text" class="form-control searching" name="query" placeholder="Search order number..." aria-label="Search" />
                ...
                if(response.status) {
                      ...
                      if(response.data.length > 0) {
                          ...
                          $.each(response.data, function(index, data) {
                              let image = data.product ?  data.product.product_image ? assetUrl + data.product.product_image.image : null : null;
                              console.log(image);
                              htmlData += `<tr>
                                <td>${index + 1}</td>
                                <td>${data.order_number}
                                </td>
                                <td>${image ? `<img src="${image}" class="img-fluid" style="max-width:80px" alt="${image}">` : ''}
                                </td>
                                ...
                                <td>
                                 ${data.status == 'new'
                                      ...
                                      : data.status ==  'process' ? `<span class="badge badge-warning">${data.status}</span>` :
                                      data.status == 'delivered' ? `<span class="badge badge-success">${data.status}</span>` :
                                      `<span class="badge badge-danger">${data.status}</span>`}
                                </td>
                                <td>
                                    <a href="{{url('admin/order/')}}/${data.id}/edit" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                    <form method="POST" action="{{url('admin/order')}}/${data.id}">
                                        @csrf
                                        @method('delete')
                         ...
                         htmlData = `<td>
                                         No order found!!! Please Enter correct order number
                                      </td>`;
                     }
