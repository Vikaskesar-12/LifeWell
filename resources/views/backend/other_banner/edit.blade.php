...
@section('title', 'LIFWEL || Banner Edit')
        ...
        <div class="card-body">
            <form method="post" action="{{ route('banners.update', $banner->id) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input id="inputTitle" type="url" name="url" placeholder="Enter url"
                        value="{{ old('url', $banner->url) }}" class="form-control">
                    @error('url')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                    ...
                    <select name="category_id" class="form-control">
                        ...
                        @foreach ($parent_cats as $key => $parent_cat)
                            <option value='{{ $parent_cat->id }}'
                                {{ $parent_cat->id == $banner->category_id ? 'selected' : '' }}>{{ $parent_cat->title }}
                            </option>
                        @endforeach
                    </select>
                    @error('category_id')
                        ...
                        <div class="col-md-6 col-sm-6">
                            <div class="image-preview mt-3" id="imagePreview">
                                <div class="image-container"
                                    ...
                                    <img src="{{ asset($banner->photo) }}" id="testimonial-preview" class="ml-2"
