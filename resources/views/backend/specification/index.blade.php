...
@section('main-content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary float-left">Specification Lists</h6>
            <a href="{{ route('specification.create') }}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip"
                data-placement="bottom" title="Add User"><i class="fas fa-plus"></i> Add Specification</a>
        </div>
        <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            function fetch_data(search, page = 1) {
                ...
                $.ajax({
                    url: '{{ route('specification.display') }}',
                    type: 'get',
                    data: {
                        search: search,
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            if (response.data.length > 0) {
                               ...
                               <td>${data.category ? data.category.title_en : ''}</td>
                               <td>${data.name }</td>
                               <td>${data.type}</td>
                               <td>
                                   <a href="{{ url('admin/specification/') }}/${data.id}/edit" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                   <form method="POST" action="{{ url('admin/specification') }}/${data.id}"">
                                       @csrf
                                       @method('delete')
                                ...
                                htmlData = `<td>
                                         No specification found!!! Please create specification
                                      </td>`;
                            }
