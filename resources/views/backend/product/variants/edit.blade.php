...
@section('title', 'LIFWEL || Add Product Variant')
@section('main-content')

    <div class="card">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary float-left">Edit Product Variant</h6>
            <a href="{{ route('product-variant', request()->segment(4)) }}" class="btn btn-primary btn-sm float-right"
                data-toggle="tooltip" data-placement="bottom" title="Add User"> Product List</a>
        </div>
        <div class="card-body">
            @if (session('error'))
                  ...
                  <button class="btn btn-success ms-auto" id="add_variant" type="button">Add Variant</button>
                    ...
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <input id="variant_title" type="text" name="title" placeholder="Enter old price"
                                value="{{ old('title', $variant->title) }}" class="form-control">
                            @error('variant_title')
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input id="discount" type="number" name="discount" min="0" max="100"
                                placeholder="Enter discount" value="{{ old('discount', $variant->discount) }}"
                                class="form-control">
                            @error('discount')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select name="collection" class="form-control">
                                ...
                                <option value="default" {{ $variant->collection == 'default' ? 'selected' : '' }}>Default
                                </option>
                    ...
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <input id="base_price" type="number" name="base_price" placeholder="Enter old price"
                                value="{{ old('base_price', $variant->base_price) }}" class="form-control">
                            @error('base_price')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input id="quantity" type="number" name="stock" min="0" placeholder="Enter quantity"
                                value="{{ old('stock', $variant->stock) }}" class="form-control">
                            @error('stock')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    @foreach ($variant->productSpecification as $key => $specification)
                        ...
                        <div class="col-md-4"> <!-- One column -->
                            <div class="form-group">
                                ...
                                <input type="text" name="{{ $specification->name }}"
                                    value="{{ $specification->type }}" required class="form-control">
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="form-group">
                    ...
                    <div id="privew">
                        @foreach ($variant->productImages as $image)
                            <div id="deleteId-{{ $image->id }}"
                                style="position: relative; display: inline-block; margin-right: 10px;">
                                <img src="{{ asset($image->image) }}" alt=""
                                    style="width: 100px; height: auto;">
                                <span class="delete-icon" data-id="{{ json_encode($image) }}"
                                    style="position: absolute; top: 0px; right: 0px; color: red; cursor: pointer; font-size: 15px; background: white; border-radius: 50%; padding: 1px;">x</span>
                            </div>
                        @endforeach
                    </div>
